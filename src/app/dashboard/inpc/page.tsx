...
const inpcTables = [
  { id: 1736, name: "INPC - Série Histórica" },
  { id: 7063, name: "INPC - Variação Mensal" },
];

export default function INPCDashboard() {
  const [selectedTable, setSelectedTable] = useState(inpcTables[0].id.toString());
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      const res = await fetchSidraData({ id: selectedTable, name: inpcTables.find(t => t.id.toString() === selectedTable)?.name }, "");
      setData(res[0].data || []);
      setLoading(false);
    }
    load();
  ...
  return (
    <div className="flex bg-[#F3F7FA] text-[#3F4D67] min-h-screen">
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <h1 className="text-2xl font-bold text-[#3F4D67]">INPC - Índice Nacional de Preços ao Consumidor</h1>
        ...
        {loading ? (
          ...
          <>
            <SidraChart title={inpcTables.find(t => t.id.toString() === selectedTable)?.name || ""} data={sortedData} firstPeriod={firstPeriod} lastPeriod={lastPeriod} />
